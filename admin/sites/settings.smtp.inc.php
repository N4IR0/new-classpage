<?php
	...
	if (isset($_POST["modify"])) {
		...
		if (isset($_POST["server"]) && !empty($_POST["server"]) && $_POST["server"] != $SETTING["server"]) {
			$sql = "UPDATE `settings` SET `value`='".mysql_real_escape_string($_POST["server"])."' WHERE `key`='smtp' AND `setting`='server'";
			if (mysql_query($sql)) {
				$changes = true;
				$changes_text .= " Server,";
			} else {
		...
		if (isset($_POST["port"]) && !empty($_POST["port"]) && $_POST["port"] != $SETTING["port"]) {
			$sql = "UPDATE `settings` SET `value`='".mysql_real_escape_string($_POST["port"])."' WHERE `key`='smtp' AND `setting`='port'";
			if (mysql_query($sql)) {
				$changes = true;
				$changes_text .= " Port,";
			} else {
		...
		if (isset($_POST["user"]) && !empty($_POST["user"]) && $_POST["user"] != $SETTING["user"]) {
			$sql = "UPDATE `settings` SET `value`='".mysql_real_escape_string($_POST["user"])."' WHERE `key`='smtp' AND `setting`='user'";
			if (mysql_query($sql)) {
				$changes = true;
				$changes_text .= " User,";
			} else {
		...
		if (isset($_POST["password"]) && !empty($_POST["password"])) {
			$sql = "UPDATE `settings` SET `value`='".mysql_real_escape_string($_POST["password"])."' WHERE `key`='smtp' AND `setting`='password'";
			if (mysql_query($sql)) {
				$changes = true;
